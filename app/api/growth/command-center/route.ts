...
export async function GET() {
  try {
    ...
    const [{ data: account }, { data: profileRow }, { data: opportunities }, { data: ideas }, { data: relationships }, { data: posts }, { data: metrics }] = await Promise.all([
      supabase.from("x_accounts").select("username,name,followers_count,following_count,tweet_count,last_successful_sync_at,sync_status,sync_error_code,updated_at").eq("app_user_id", appUserId).maybeSingle(),
    ...
    const realPosts = (posts ?? []).filter((p) => p.data_source !== "demo").map((p) => {
    ...
    return NextResponse.json({
      account,
      goal: profile.target_followers,
      progress: { followers: followerCount, remaining: Math.max(0, profile.target_followers - followerCount), percent: Math.min(100, Number(((followerCount / Math.max(1, profile.target_followers)) * 100).toFixed(2))) },
      actions,
      opportunities: rankedOpportunities,
      content: rankedIdeas,
      relationships: relationships ?? [],
      learning,
      dataStatus: { x: account?.sync_status ?? "unavailable", realPosts: realPosts.length, demoAvailable: true, lastSuccessfulSyncAt: account?.last_successful_sync_at ?? null },
      metrics: metrics ?? [],
      postSignals: realPosts.map((p) => ({ ...classifyPost(p.text), engagements: p.engagements, impressions: p.impressions })),
    });
  } catch (error) {
