...
function nextAction(count: number, lastInteraction: string | null, topics: string[]) {
  if (!lastInteraction) return "Start a genuine conversation around a relevant topic.";
  const days = Math.floor((Date.now() - new Date(lastInteraction).getTime()) / 86400000);
  if (days >= 14) return topics[0] ? `Reconnect around ${topics[0]}.` : "Reconnect with a useful question or update.";
  if (days >= 7) return topics[0] ? `Continue the conversation around ${topics[0]}.` : "Continue the conversation with something useful.";
  return "Keep the relationship natural; respond when you have something useful to add.";
}
...
export async function GET() {
  try {
    ...
    return NextResponse.json({ relationships: (data ?? []).sort((a: any, b: any) => relationshipPriority(b) - relationshipPriority(a)) });
  } catch (error) {
    const message = error instanceof Error ? error.message : "Could not load relationships";
...
export async function POST(request: Request) {
  try {
    ...
    const username = String(body.x_username || "").replace(/^@/, "").trim().slice(0, 100);
    ...
    const topics = Array.isArray(body.topics) ? body.topics.filter((v: unknown): v is string => typeof v === "string").map((v) => v.trim()).filter(Boolean).slice(0, 20) : [];
    const last = body.last_interaction_at || null;
...
export async function PATCH(request: Request) {
  try {
    ...
    if (typeof body.niche === "string") update.niche = body.niche.slice(0, 300);
    if (typeof body.display_name === "string") update.display_name = body.display_name.slice(0, 200);
    if (Array.isArray(body.topics)) update.topics = body.topics.filter((v: unknown): v is string => typeof v === "string").map((v) => v.trim()).filter(Boolean).slice(0, 20);
    if (typeof body.last_interaction_at === "string") update.last_interaction_at = body.last_interaction_at;
    ...
    const current = await supabase.from("relationships").select("interaction_count,last_interaction_at,topics").eq("id", body.id).eq("app_user_id", appUserId).maybeSingle();
    const count = Number(update.interaction_count ?? current.data?.interaction_count ?? 0);
    const last = String(update.last_interaction_at ?? current.data?.last_interaction_at ?? "") || null;
    const topics = Array.isArray(update.topics) ? update.topics as string[] : (current.data?.topics ?? []);
    update.last_topic = topics[0] || "";
    update.next_action = nextAction(count, last, topics);
