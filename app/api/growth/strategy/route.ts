...
async function loadData(appUserId: string, supabase: any) {
  const { data: account, error: accountError } = await supabase.from("x_accounts").select("x_user_id,username,name,description,profile_image_url,followers_count,following_count,tweet_count,last_successful_sync_at,sync_status,sync_error_code,updated_at").eq("app_user_id", appUserId).maybeSingle();
  if (accountError) throw new Error(accountError.message);
  const [{ data: profileRow }, { data: opportunities }, { data: ideas }, { data: relationships }, { data: drafts }, { data: posts }, { data: metrics }, { data: projects }, { data: research }] = await Promise.all([
...
export async function GET() {
  try { const { appUserId, supabase } = await requireAppUser(); return NextResponse.json(buildStrategy(await loadData(appUserId, supabase))); }
  catch (error) { const message = error instanceof Error ? error.message : "Could not build growth strategy"; return NextResponse.json({ error: message }, { status: message === "UNAUTHENTICATED" ? 401 : 500 }); }
}
